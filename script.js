...
function exitGuestMode() {
    ...
    if (confirm('确认退出访客模式？')) {
        ...
        isGuestMode = false;
        guestProducts = [];
        guestFilteredProducts = [];
        
        // 清空搜索和筛选
        document.getElementById('guestSearchInput').value = '';
        document.getElementById('guestCategoryFilter').value = '';
...
// 创建模拟访客数据（用于演示）
function createMockGuestData() {
    return [
        { name: '西门子断路器 3UA', specification: '16A 3P 400V', sell_price: 285.00, category_name: '断路器' },
        { name: '施耐德接触器 LC1D', specification: '25A 220V AC', sell_price: 156.00, category_name: '接触器' },
        { name: 'ABB 小型断路器 S201', specification: '10A 1P C型', sell_price: 42.00, category_name: '断路器' },
        { name: '正泰时间继电器 JSS48A', specification: '220V AC', sell_price: 68.00, category_name: '继电器' },
        { name: '德力西空气开关 DZ47', specification: '32A 2P', sell_price: 35.00, category_name: '开关' },
        { name: '欧姆龙中间继电器 MY2N', specification: '24V DC', sell_price: 28.00, category_name: '继电器' },
        { name: '施耐德按钮开关 XB2', specification: '红色 常开', sell_price: 18.00, category_name: '开关' },
        { name: '西门子接触器 3TF', specification: '40A 220V', sell_price: 198.00, category_name: '接触器' },
        { name: 'ABB 漏电保护器 DS201', specification: '16A 30mA', sell_price: 125.00, category_name: '断路器' },
        { name: '正泰交流接触器 NC1', specification: '18A 220V', sell_price: 45.00, category_name: '接触器' }
    ];
}

// 设置访客模式监听器
function setupGuestModeListeners() {
    const searchInput = document.getElementById('guestSearchInput');
    const categoryFilter = document.getElementById('guestCategoryFilter');
    ...
    // 分类筛选
    if (categoryFilter) {
        categoryFilter.addEventListener('change', performGuestFilter);
    }
}
...
// 执行访客模式筛选
function performGuestFilter() {
    const category = document.getElementById('guestCategoryFilter').value;
    const searchQuery = document.getElementById('guestSearchInput').value.trim().toLowerCase();
    
    guestFilteredProducts = guestProducts.filter(product => {
        // 分类筛选
        if (category && product.category_name !== category) {
            return false;
        }
...
function renderGuestProducts() {
    const container = document.getElementById('guestProductList');
    
    if (guestFilteredProducts.length === 0) {
        container.innerHTML = `
            <div class="guest-no-results">
                <svg class="guest-no-results-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
                <div class="guest-no-results-title">未找到相关商品</div>
                <div class="guest-no-results-text">请尝试其他关键词</div>
            </div>
        `;
        return;
    }
    
    const html = guestFilteredProducts.map(product => `
        <div class="guest-product-card">
            <div class="guest-product-name">${product.name}</div>
            ${product.specification ? `<div class="guest-product-spec">${product.specification}</div>` : ''}
            <div class="guest-product-price">${product.sell_price.toFixed(2)}</div>
        </div>
    `).join('');
...
function updateGuestStats() {
    const statsCount = document.getElementById('guestStatsCount');
    const total = guestProducts.length;
    const showing = guestFilteredProducts.length;
    
    if (showing === total) {
        statsCount.textContent = `共 ${total} 件商品`;
        statsCount.className = 'stats-count';
    } else if (showing === 0) {
        statsCount.textContent = `未找到商品`;
        statsCount.className = 'stats-count no-results';
    } else {
        statsCount.textContent = `找到 ${showing} 件商品`;
        statsCount.className = 'stats-count has-results';
    }
}
